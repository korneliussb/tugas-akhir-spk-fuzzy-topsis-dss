...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 11 Bulan Mei 2020 pada 16.59
...
(6, 'Harga air (Cost)', 0.00, 0.30, 0.60, 1),
...
(2, 'Yayaya', 'yayaya', '$2y$10$V7tX5aLG/S1PL79VkCW9uOoDiwk2Qv1uo14bp8fj/9JdnC26ZrGCy', 1, 'yaya1@gmail.com', 1, 'UND', '2020-03-16 10:13:53'),
...
(16, 'User Biasa', 'userbeaja', '$2y$10$/oFuFuqtYja81mt2.ds8nOKRV0s7uLocIZnxc0DpS451x5AjpsJdG', 2, 'useraja@gmail.com', 1, 'Politeknik Keuangan Negara', '2020-04-15 02:17:35'),
(17, 'User', 'useraja', '$2y$10$Cf0uNVecFMWYkV35dfja5OOi23nWqQFMCQeh5XZCFgqgdtbJJ.K2O', 2, 'user@gmail.com', 2, 'UNIV', '2020-04-15 07:33:45'),
(18, 'Manajer Indonesia', 'manajer', '$2y$10$j3bF9bpoefvfL1qpKUNYyOzSdziXES8Da4tjGVYfwFx7RB4mCoxHq', 2, 'manajer@gmail.com', 1, 'PT Manajer Indonesia', '2020-04-20 04:26:35');
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_sum`  AS  select `v_sumneg`.`id_alternatif` AS `id_alternatif`,`v_sumneg`.`dneg` AS `dneg`,`v_sumplus`.`dplus` AS `dplus` from (`v_sumneg` join `v_sumplus`) where `v_sumplus`.`id_alternatif` = `v_sumneg`.`id_alternatif` ;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_sumneg`  AS  select `v_dneg`.`id_alternatif` AS `id_alternatif`,sum(`v_dneg`.`dneg`) AS `dneg` from `v_dneg` group by `v_dneg`.`id_alternatif` ;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_sumplus`  AS  select `v_dplus`.`id_alternatif` AS `id_alternatif`,sum(`v_dplus`.`dplus`) AS `dplus` from `v_dplus` group by `v_dplus`.`id_alternatif` ;
...
--
ALTER TABLE `alternatif`
  MODIFY `id_alternatif` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;

--
-- AUTO_INCREMENT untuk tabel `aspek_teknik`
--
ALTER TABLE `aspek_teknik`
  MODIFY `id_aspek_teknik` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=232;
